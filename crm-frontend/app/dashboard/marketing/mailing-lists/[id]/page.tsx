...
export default function EditMailingListPage() {
  const router = useRouter()
  const params = useParams()
  const listId = parseInt(params.id as string)

  const { data: mailingList, isLoading: isLoadingList } = useMailingList(listId)
  const { updateList, isUpdating } = useMailingLists()
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-text-secondary mt-1">
              Modifiez les informations et la sélection de destinataires
            </p>
          </div>
        </div>
        <Button
          ...
          disabled={isUpdating || !formData.name.trim() || recipientCount === 0}
        >
        ...
        <CardBody className="space-y-spacing-md">
          ...
          <div className="pt-4 border-t border-border">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-text-tertiary">Créée le:</span>
                <span className="ml-2 font-medium text-text-primary">
                  {new Date(mailingList.created_at).toLocaleDateString('fr-FR', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                  })}
              ...
              {mailingList.last_used_at && (
                <div>
                  <span className="text-text-tertiary">Dernière utilisation:</span>
                  <span className="ml-2 font-medium text-text-primary">
                    {new Date(mailingList.last_used_at).toLocaleDateString('fr-FR', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}
