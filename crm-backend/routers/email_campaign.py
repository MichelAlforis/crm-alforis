...
def update_email_template(
    template_id: int, template_update: EmailTemplateUpdate, db: Session = Depends(get_db)
):
...
def send_test_email_from_template(template_id: int, test_email: str, db: Session = Depends(get_db)):
...
def list_email_campaigns(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def update_email_campaign(
    campaign_id: int, campaign_update: EmailCampaignUpdate, db: Session = Depends(get_db)
):
...
def prepare_email_campaign(campaign_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"message": f"Campaign prepared successfully", "emails_prepared": count}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def get_campaign_statistics(campaign_id: int, db: Session = Depends(get_db)):
...
def start_email_campaign(campaign_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        emails_count = (
            db.query(CampaignEmail).filter(CampaignEmail.campaign_id == campaign_id).count()
        )

        if emails_count == 0:
            raise HTTPException(
                status_code=400, detail="No emails prepared. Please prepare the campaign first."
            )
...
def pause_email_campaign(campaign_id: int, db: Session = Depends(get_db)):
...
def count_recipients(filters: RecipientFilters, db: Session = Depends(get_db)):
...
def send_test_email(campaign_id: int, test_email: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"message": "Test email sent successfully", "to_email": test_email}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to send test email: {str(e)}")
